...
class LoginCubit extends Cubit<LoginState> {
  ...
  void emailChanged(String email) {
    emit(
      state.copyWith(
        email: email,
        emailError: (state.hasSubmitted || state.emailTouched)
  ...
  void passwordChanged(String password) {
    emit(
      state.copyWith(
        password: password,
        passwordError: (state.hasSubmitted || state.passwordTouched)
  ...
  Future<void> login() async {
    ...
    try {
      ...
      final pendingCompanyId = await _sessionCubit
          .consumePendingRequestCompanyId();
      final postLoginRoute = pendingCompanyId == null
          ? null
          : AppRouter.newServiceRequestLocation(companyId: pendingCompanyId);
  ...
  void _startRateLimitCooldown() {
    ...
    _cooldownTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (remaining <= 0) {
        timer.cancel();
        emit(
          state.copyWith(isRateLimited: false, rateLimitRemainingSeconds: 0),
        );
      } else {
  ...
  ({LoginErrorCode code, LoginErrorCategory category}) _extractErrorCode(
    DioException e,
  ) {
    if (e.response != null) {
      ...
      if (statusCode == 200 || statusCode == 201) {
        return (
          code: LoginErrorCode.serverError,
          category: LoginErrorCategory.server,
        );
      }
      ...
      switch (statusCode) {
        case 400:
          return (
            code: LoginErrorCode.accountNotSetup,
            category: LoginErrorCategory.accountLink,
          );
        case 401:
          return (
            code: LoginErrorCode.invalidCredentials,
            category: LoginErrorCategory.credentials,
          );
        case 403:
          return (
            code: LoginErrorCode.serverError,
        ...
        case 429:
          return (
            code: LoginErrorCode.rateLimit,
            category: LoginErrorCategory.rateLimit,
          );
        case 500:
        case 502:
        case 503:
        ...
        default:
          return (
            code: LoginErrorCode.unexpectedError,
            category: LoginErrorCategory.server,
          );
      }
    }
    ...
    switch (e.type) {
      ...
      case DioExceptionType.connectionError:
        return (
          code: LoginErrorCode.networkError,
          category: LoginErrorCategory.network,
        );
