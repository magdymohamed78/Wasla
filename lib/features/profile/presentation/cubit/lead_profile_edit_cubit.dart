...
import 'lead_profile_edit_state.dart';

class LeadProfileEditCubit extends Cubit<LeadProfileEditState> {
  final GetLeadProfileUseCase _getLeadProfileUseCase;
  final UpdateLeadProfileUseCase _updateLeadProfileUseCase;
  final SessionCubit _sessionCubit;

  LeadProfileEditCubit({
    required GetLeadProfileUseCase getLeadProfileUseCase,
    required UpdateLeadProfileUseCase updateLeadProfileUseCase,
    required SessionCubit sessionCubit,
  }) : _getLeadProfileUseCase = getLeadProfileUseCase,
       _updateLeadProfileUseCase = updateLeadProfileUseCase,
       _sessionCubit = sessionCubit,
       super(const LeadProfileEditState());

  Future<void> load() async {
    emit(
      state.copyWith(
        status: LeadProfileEditStatus.loading,
    ...
    try {
      ...
      emit(
        state.copyWith(
          status: LeadProfileEditStatus.initial,
          profile: profile,
      ...
      emit(
        state.copyWith(
          status: LeadProfileEditStatus.failure,
  ...
  Future<void> save() async {
    ...
    emit(
      state.copyWith(
        status: LeadProfileEditStatus.saving,
        clearErrorMessage: true,
        hasSubmitted: true,
      ),
    );
    ...
    try {
      final updated = await _updateLeadProfileUseCase(
      ...
      emit(
        state.copyWith(
          status: LeadProfileEditStatus.success,
          profile: updated,
