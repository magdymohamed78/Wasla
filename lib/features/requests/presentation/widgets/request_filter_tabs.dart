...
class RequestFilterTabs extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final filters = [
      (filter: RequestFilter.all, label: localizations.requestsFilterAll),
      (
        filter: RequestFilter.pending,
        label: localizations.requestsFilterPending,
      ),
      (
        filter: RequestFilter.offerSent,
        label: localizations.requestsFilterOfferSent,
      ),
      (
        filter: RequestFilter.declined,
        label: localizations.requestsFilterDeclined,
      ),
      (
        filter: RequestFilter.expired,
        label: localizations.requestsFilterExpired,
      ),
    ];
      ...
      child: SizedBox(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            final count = counts[entry.filter] ?? 0;

            return _AnimatedFilterChip(
              label: entry.label,
              count: count,
              isActive: isActive,
              activeColor: RequestFilter.resolveColor(entry.filter),
